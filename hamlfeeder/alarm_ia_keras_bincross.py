        ...
        with open(os.path.join(dir_path, 'entities.dat')) as fo:
            ...
            for entity in fo:
                ...
                if response.status_code == 200:
                    ...
                    state['last_changed'] = parse(
                        state['last_changed']) + timedelta(hours=1)
                    states.append(state)
            return get_normalized(pd.DataFrame([format_data(states)]))


if __name__ == '__main__':
    print("Utilizando modelo TF/Keras")
    alarm_state = 'Apagada'
    msg = "La alarma debería estar {}"
    _X = request_live_data()
    model = load_model(os.path.join(saves_path, 'keras_bincross_model.h5'))
    X = tf.convert_to_tensor(_X.values, dtype=tf.float64)
